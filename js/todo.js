  ...
  sort(field, order) {
    this.items.sort((item1, item2) => {
      switch(field) {
        ...
        default:
          return order === "desc" ? item2.seq - item1.seq : item1.seq - item2.seq;
      }
    });
    this.render();
  }
...
window.addEventListener("DOMContentLoaded", function() {
    ...
    try{
      ...
      for (const [field, message] of Object.entries(requiredFields)) {
        ...
        if (field === 'deadline' && new Date(value) - new Date() < 0) {
          throw new Error(JSON.stringify({field, message:"현재 날짜 이후로 입력하세요."}));
        }
  ...
  frmSearch.sort.addEventListener("change", function() {
    const [ field, order ] = this.value.split("_"); // 비구조 할당
  ...
  frmSearch.skey.addEventListener("change", function() {
    ...
    todo.itemsSearched = skey ? todo.items.filter(({title, description}) => title.includes(skey) || description.includes(skey)) : null;
